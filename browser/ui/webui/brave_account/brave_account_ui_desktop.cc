...
void BraveAccountUIDesktop::GetDialogMode(GetDialogModeCallback callback) {
  std::move(callback).Run(brave_account::DialogModeHolder::Get(
...
void ShowBraveAccountDialog(content::WebUI* web_ui,
  ...
  // not the opener's. `BraveAccountUIDesktop::GetDialogMode()` reads it back
  // when the page asks.
  brave_account::DialogModeHolder::Set(CHECK_DEREF(delegate.GetWebContents()),
                                       dialog_mode);
