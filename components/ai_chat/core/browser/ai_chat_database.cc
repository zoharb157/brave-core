...
bool MigrateFrom9to10(sql::Database* db) {
  // Check if column already exists — CreateSchema() may have already created
  // the table with this column for databases older than v4 (which didn't have
  // the conversation_entry_uploaded_files table at all).
  if (db->DoesColumnExist("conversation_entry_uploaded_files",
                          "extracted_text")) {
    return true;
  }
...
std::vector<mojom::ConversationTurnPtr> AIChatDatabase::GetConversationEntries(
  ...
  while (statement.Step()) {
    ...
    while (uploaded_file_statement.Step()) {
      ...
      auto extracted_text =
          DecryptOptionalColumnToString(uploaded_file_statement, 4);
...
bool AIChatDatabase::AddConversationEntry(
  ...
  if (entry->uploaded_files.has_value()) {
    for (size_t i = 0; i < entry->uploaded_files->size(); ++i) {
      ...
      BindAndEncryptOptionalString(uploaded_file_statement, 5,
...
bool AIChatDatabase::AddConversationThread(mojom::ThreadPtr thread) {
  ...
  static constexpr char kInsertThreadQuery[] =
      ...
      " origin_conversation_entry_uuid, total_tokens, trimmed_tokens)"
      " VALUES(?, ?, ?, ?, ?)";
...
bool AIChatDatabase::ApplyRemoteEntry(
  ...
  // Upsert the per-entry associated_content metadata together with the
  // caller-supplied content texts. The caller is responsible for filling in
  // local values for any AC the remote sender omitted to fit the size budget.
