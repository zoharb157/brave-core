...
public class NTPDataSource {
  ...
  func getImageBackground() -> NTPWallpaper? {
    ...
    // Scout shows its own artwork only, never Brave's downloaded photography.
    let backgroundSet = NTPBackgroundImage.scoutWallpapers.map(NTPWallpaper.image)
...
extension NTPBackgroundImage {
  /// Scout's new-tab artwork. Three abstract night skies on one violet base,
  /// each carrying a different low accent — dusk amber, mint aurora, violet
  /// starfield — and each deliberately dark and quiet across the top, where
  /// the Protection card and the status bar sit over them.
  ///
  /// Made with gpt-image-2 and fitted to 1290x2796 by extending the sky
  /// upward rather than cropping: the frames come back at 2:3 and the phone
  /// wants 1:2.17, so a crop would have discarded most of the width and then
  /// needed an 1.8x upscale to recover it. They replace procedurally drawn
  /// gradients. No author, so the NTP shows no photo credit.
  static let scoutWallpapers: [NTPBackgroundImage] = ["horizon", "aurora", "constellation"].map {
    .init(
      imagePath: Bundle.module.url(forResource: "scout-wallpaper-\($0)", withExtension: "jpg")!,
      author: "",
      link: ScoutContact.site
    )
  }

  static let fallback: NTPBackgroundImage = scoutWallpapers[0]
}
